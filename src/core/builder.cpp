...
private:
	...
	{
		// Unused lanes stay at their zero-initialized value below, which gives them
		// zero-length edges. bvh8.cpp's segment_blocked cached-packet check relies on
		// this: it tests all 8 lanes of a cached packet regardless of the leaf's real
		// triangle count, and only stays correct because a zero-edge lane can never
		// pass hit_packet's determinant check. Keep lanes beyond the real count zeroed
		// if this function ever changes.
